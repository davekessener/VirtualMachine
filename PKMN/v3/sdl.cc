...
namespace sdl
{
	...
	void start(const std::string& wn, int w, int h, bool full)
	{
		...
		bool running = true;
    	Uint32 start, delta(1);
    	SDL_Event e;

		std::map<int, MouseButtons> SDLToDAV;
		SDLToDAV[SDL_BUTTON_LEFT] = MouseButtons::LEFT;
		SDLToDAV[SDL_BUTTON_RIGHT] = MouseButtons::RIGHT;
		SDLToDAV[SDL_BUTTON_MIDDLE] = MouseButtons::MIDDLE;
		...
		while(running)
		{
    	    ...
    	    while(SDL_PollEvent(&e)) switch(e.type)
			{
				...
				case SDL_MOUSEBUTTONUP:
					if(m_input_fn)
					{
						m_input_fn(SDLToDAV[e.button.button], e.button.x, e.button.y, e.type == SDL_MOUSEBUTTONDOWN);
					}
					break;
    	    }
